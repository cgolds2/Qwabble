...
namespace groupProject01
{
    public class CalendarData
    {
        ...
        public void getEventsInCalendar(int CalendarID)                     //gets all events for a specific calendar
        {
            ...
            //HARD-CODED, WILL CHANGE
            events.Add(new EventObject { eventName = "AppearOnAll", ID = events.Count, senderID = 1, startDate= todayWithTime, endDate= todayWithTime, AdditionalInfo = "HELLO I AM ADDITIONAL INFO WHO ARE YOU" });     //adds the first event to the page
            if(CalendarID == 0)                                 //adds the following events to the View Calendar page
            {
                events.Add(new EventObject { eventName = "Party", ID = events.Count, senderID = 0, startDate = todayWithTime, endDate = todayWithTime, AdditionalInfo = "haha. connor throwing a party." });     //adds the  event to the View Calendar page
            }
            if(CalendarID == 1)                                                            //adds the following options to the New Event page
            {
                events.Add(new EventObject { eventName = "Schedule New Guests", ID = events.Count, senderID = 1, startDate = todayWithTime, endDate = todayWithTime, AdditionalInfo = "" });        //adds the Schedule New Guests to the New Event page
                events.Add(new EventObject { eventName = "Schedule Chore", ID = events.Count, senderID = 1, startDate = todayWithTime, endDate = todayWithTime, AdditionalInfo = "" });             //adds the Schedule Chore to the New Event page
                events.Add(new EventObject { eventName = "Reserve Laundry Machines", ID = events.Count, senderID = 0, startDate = todayWithTime, endDate = todayWithTime, AdditionalInfo = "" });   //adds the Reserve Laundry Machines to the New Event page
                events.Add(new EventObject { eventName = "Reserve Quiet Time", ID = events.Count, senderID = 1, startDate = todayWithTime, endDate = todayWithTime, AdditionalInfo = "" });         //adds the Reserve Quiet Time to the New Event page
            }
        }
    }
